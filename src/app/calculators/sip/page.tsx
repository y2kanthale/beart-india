...
import { ResponsiveContainer, BarChart as RechartsBarChart, Bar, XAxis, YAxis, Tooltip as RechartsTooltip, Legend as RechartsLegend, Cell } from 'recharts'; // Added Cell
...
export const SipCalculator = () => {
  ...
  const calculateSip = () => {
    if (!isMounted || monthlyInvestment <= 0 || expectedReturnRate < 0 || timePeriod <= 0) { // Allow 0% return rate
        setTotalInvested(0);
    ...
    if (r === 0) { 
        ...
        setTotalValue(Math.round(investedAmount));
         const dataForChart: BarChartDataPoint[] = [];
    ...
    for (let year = 1; year <= timePeriod; year++) {
      ...
      if (r === 0) { // This condition is now effectively handled above, but keep for safety.
  ...
  return (
    <Card className="w-full max-w-2xl mx-auto shadow-xl animate-in fade-in zoom-in-95 duration-500 border-0 shadow-none">
      ...
      <CardContent className="space-y-6 sm:space-y-8">
        ...
        {barChartData.length > 0 && (totalInvested > 0 || totalValue > 0) && (
          <div className="mt-6 sm:mt-8 h-[250px] sm:h-[300px] w-full animate-in fade-in duration-500 delay-200">
            <ResponsiveContainer width="100%" height="100%">
              <RechartsBarChart data={barChartData} margin={{ top: 5, right: 0, left: chartMarginLeft, bottom: 5 }}>
