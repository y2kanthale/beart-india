...
// Input not used directly in this component's JSX
...
// Renamed to EmiCalculator and made exportable
export const EmiCalculator = () => {
  const [loanAmount, setLoanAmount] = useState(2000000); // Example: 20 Lakhs
  const [interestRate, setInterestRate] = useState(8.5); // Example: 8.5%
  const [loanTenure, setLoanTenure] = useState(20); // Example: 20 Years
  ...
  const calculateEmi = () => { // calculateEmi is now stable
    if (!isMounted || loanAmount <= 0 || interestRate < 0 || loanTenure <= 0) { // Allow 0 interest rate
      setEmi(0);
      setTotalInterest(0);
      setTotalPayment(0);
      setChartData([]);
    ...
    if (r === 0) { 
        ...
        setEmi(n > 0 ? Math.round(calculatedEmi) : 0); // Avoid division by zero if tenure is 0
        setTotalInterest(0);
    ...
    if (isNaN(emiValue) || !isFinite(emiValue)) { // Check for NaN or Infinity
        setEmi(0);
        setTotalInterest(0);
        setTotalPayment(0);
        setChartData([]);
  ...
  if (!isMounted) {
    return <div className="flex justify-center items-center h-64">Loading calculator...</div>; // Or a skeleton loader
  }
...
export default function EmiCalculatorPageContainer() {
  return (
